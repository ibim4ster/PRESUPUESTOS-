...
export const Dashboard: React.FC<DashboardProps> = ({ onEditBudget, onNewBudget, currentSystem }) => {
  ...
  useEffect(() => {
    ...
    return unsubscribe;
  }, [currentSystem]); // Reload when system changes

  const loadData = () => {
    // Filter budgets by system (default to agora if undefined for backward compatibility)
              ...
              {budgets.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-8 py-16 text-center text-slate-400 flex flex-col items-center gap-3">
                    ...
                    <span className="font-medium">No hay presupuestos de {currentSystem.toUpperCase()} creados aún.</span>
